...
databaseDestinationDir = './database_binaryMask_CNNfeature_globalContrastNorm_WholeImgVGG19LastLayers';
...
for categoryID = 1:numel(categoryList)
    ...
    for imIDX = 1:numel(imList)
        ...
        % get binary mask for later use
        imMask = genMask(im);
        
        % select local patches
%         [patchLoc, imMask] = genCanonicalPatches(imBKUP, patchSize, patchNum, overlapBorder);
        ...
        res = pollenFeatureByCNN(net, im_, 43) ;
...
%                 end
                ...
                save(filename, 'imFeaMap', 'imMask', 'feaSize','imSize');
