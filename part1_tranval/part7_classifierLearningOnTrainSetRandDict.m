...
smallK = 600; % {300, 512, 600} extract first smallK atoms as the dictionary rather than the whole
lambdaL = 0.01;
...
dictName = ['randExemplarDict_K' num2str(K) 'L' num2str(lambdaL) '_D' num2str(lambdaD) '_E' num2str(lambdaE) '_B' num2str(lambdaB) '_globalContrastNorm.mat' ];
load(dictName);
...
for i_img = 1:numTrainData
    ...
    locFeatTrain{i_img} = [min(patchLoc,[],2); max(patchLoc,[],2); max(sqrt(sum(patchLoc.^2))); polyArea; varFea(:); ]; %max(score',[],2); min(score',[],2)  ];
...
for i_img = 1:numel(testData) % 1:numel(testData)
    ...
    locFeatVal{i_img} = [min(patchLoc,[],2); max(patchLoc,[],2); max(sqrt(sum(patchLoc.^2))); polyArea; varFea(:);]; % max(score',[],2); min(score',[],2) ];
...
model = libsvm_svmtrain(trainClassLabel', scXTrain', '-c 27 -t 0'); % linear 
...
%% save results
save( ['./result0_randDict/valscFeatOnExemplar_layer' num2str(curLayer) '_' shapeType '_T' num2str(T) '_lambda' num2str(lambda) '_ResultACC' num2str(bestAcc) '_globalContrastNorm.mat'], ...
    'bestAcc', 'alphaIdx',...       
