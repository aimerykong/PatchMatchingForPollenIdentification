...
lambda1 = 0.16; % weight for sparsity

%{
----> lambda1
78.1022%(0.1), 80.7299%(0.15), 80.7299%(0.16), 80.8759%(0.17),
80.7299%(0.18), 81.8978%(0.2), 78.1022%(0.25),74.5985%(0.3)  

----lambda1=0.01 ---> locationScaleFactor
77.2263%(0.0005),81.6058%(0.0008), 82.0438%(0.001), 80.438%(0.0012),
79.708%(0.0015), 80.8759%(0.002), 81.6058%(0.0025), 76.6423%(0.003),
71.3869%(0.004) 

--- SVM C=27 locationScaleFactor=0.001 ---> lambda1
82.1898%(0.14), 82.1898%(0.15), 82.4818%(0.16), 82.3358%(0.17), 81.4599%(0.18), 

-- SVM C=27  lambda1=0.16 ---> locationScaleFactor
81.8978%(0.0009), 82.4818%(0.001), 81.6058%(0.0011)
%}
...
fprintf('sparse-coding based accuracy\n');
fprintf('accuracy=%.4f (lambda1=%.4f, lambdaLoc=%.4f, T=%d)\n\n', acc, lambda1, lambdaLoc, T);
for categID = 1:numel(pollenName)
...
model = libsvm_svmtrain(trainClassLabel', scXTrain', '-s 0 -c 27 -t 0'); % linear 
...
fprintf('\nlinear Regression best overall acc: %.4f, alpha=%.4f\n', bestAcc, alphaList(alphaIdx));
