...
K = 36;  % select K desired receptive fields

lambdaD = 0.5; % discriminative term (2) -- make the selected points more "class-pure" 
...
lambdaB = 1.5; % balance term (4) -- make the size of clusters represented by selected point similar
...
while flag  
    ...
    % plus equal-size term (third one)
    term3 = sum(log(1+CountNumRFperImg));
    Hcur = Hcur + lambdaE*term3;        
    
    % plus balance cluster size term (forth term)
    term4 = 0;
    tmpColumns = SimilarityGraph_backup(:, SelectAlready);  
    ...
    term4 = term4  - length(SelectAlready);
    Hcur = Hcur + lambdaB*term4; 
    ...
    predictH = predictH + lambdaE*term3List;
        
    % plus balance cluster size term (forth term)
    term4List = zeros(1, length(SelectNotYet));
    for j = 1:length(SelectNotYet)
        tmpColumns = SimilarityGraph_backup(:, PredictSelectSet(:,j));
        [~, simCluster] = max(tmpColumns,[],2);
        term4 = 0;
        for i = 1:length(PredictSelectSet(:,j))
            tmp = length(find(simCluster==i));
        ...
        term4 = term4  - length(PredictSelectSet(:,j));
        term4List(j) = term4;
    end
    predictH = predictH + lambdaB*term4List;
    
    fprintf('\titer-%d, [term1=%.4f term2=%.4f term3=%.4f, term4=%.4f]\n', k, max(term1List), lambdaD*max(term2List), max(lambdaE*term3List), max(lambdaB*term4List) );
