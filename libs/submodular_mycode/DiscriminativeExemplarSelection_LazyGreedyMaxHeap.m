function [exemplarIndex] = DiscriminativeExemplarSelection_LazyGreedyMaxHeap(SimilarityGraph, LocationAffinityGraph, FullConnectGraph, labelList, K, lambdaL, lambdaD, lambdaE, lambdaB)
...
HeapGainList = MaxHeap(length(gainList), gainList);
...
while flag  
    ...
    Delta_e = HeapGainList.ReturnMax();
    ei = find(gainList(SelectNotYet)==Delta_e);
    ei = ei(1);
    ...
    if ei == preSelectID
        ...
        HeapGainList.ExtractMax();
        HeapGainList.InsertKey(delta);
    end
    
    %% checking and updating
    if delta >= HeapGainList.ReturnMax()
        if delta <= 0
            break;
        else            
            k = k + 1;
