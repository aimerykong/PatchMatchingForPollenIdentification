...
lambdaB = 10; % balance term -- a very large lambda1 will lead to that the points are being selected from class to class

ChoiceGraphBuilding = 'nothing'; % which way to build the graph, overall_knn or individual_knn or nothing
...
switch ChoiceGraphBuilding
    case 'overall_knn',
        for i = 1:size(SimilarityGraph, 2)
            ...
            SimilarityGraph(i, :) = 0;
            SimilarityGraph(i, costIdx(1:knn*25)) = costListSorted(1:knn*25);
        end
    case 'individual_knn',
        for i = 1:size(SimilarityGraph, 2)
            idx = imgIdx(i);
...
while flag    
    ...
    Hcur = log(Hcur) + lambdaB*sum(log(1+CountNumRFperImg));
    ...
    predictH = log( H_A' ) + lambdaB*sum( log(1+g_balance), 1 );
