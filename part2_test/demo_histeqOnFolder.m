...
destFolder = './globalNormalizationImages';
if ~isdir(destFolder)
    mkdir(destFolder);
end

imList = dir( fullfile(datasetDir,'*.jpg') );
...
for i = 1:length(imList)
    ...
    im = imread( fullfile(datasetDir, filename) );
    
    subplot(1,3,1);
    imshow(im);
    title(['max value:' num2str(max(im(:)))]);
        
    im2 = im;
    idx = find(im2>0);
    im2(idx) = histeq(im2(idx), histRecord);

%     im2 = single(im) ./ single(max(im(:)));
    subplot(1,3,2);
    imshow((im2))
    title(['max value:' num2str(255*max(im(:)))]);
    ...
    export_fig( fullfile(destFolder, filename)); % '-transparent', ,  '-png'
end
