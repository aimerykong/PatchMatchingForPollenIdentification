...
locationScaleFactorList = [0.0008:0.0002:0.0012, 0.002];

T = 45; % nonzero elements default 40
lambdaLoc = 0; % weight of location-aware penalty default 0.09
lambda1List = [0.007, 0.01:0.02:0.08, 0.1, 0.2]; % weight for sparsity
...
tic
for sclrID = 1:length(locationScaleFactorList)
    locationScaleFactor = locationScaleFactorList(sclrID);
    for lambda1ID = 1:length(lambda1List)
        lambda1 = lambda1List(lambda1ID);
        ...
        for categID = 1:length(pollenName)
            ...
            for i_img = 1:numTrain
                count = count + 1;
                fprintf('-%d/%d ', count, numTrainData );
        ...
        for categID = 1:length(pollenName)
            ...
            for i_img = numTrain+1:length(tmpMat.imList)
                count = count + 1;
                fprintf('-%d/%d ', i_img, length(tmpMat.imList) );
        ...
        model = libsvm_svmtrain(trainClassLabel', scXTrain', '-s 0 -c 50 -t 0'); % linear
        ...
        %% save results
        save( ['./result5_wholeImage/valscFeatOnExemplar_layer' num2str(curLayer) '_' shapeType '_locationScaleFactor' num2str(locationScaleFactor) '_lambda1' num2str(lambda1) '_SVMAcc' num2str(SVMAcc) '.mat'], ...
            'bestAcc', 'alphaIdx',...
